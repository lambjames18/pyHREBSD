...
os.mkdir(up2_save_folder, exist_ok=True)
for i in tqdm(ii, desc="Exporting patterns", unit="pats"):
    pat = np.squeee(ebsd_obj.pat_reader(i, 1))
    pat = np.around(255 * (pat - pat.min()) / (pat.max() - pat.min())).astype(np.uint8)
    io.imsave(f"{up2_save_folder}{i}.png", pat)